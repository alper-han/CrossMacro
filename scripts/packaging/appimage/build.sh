...
if [ "$APPIMAGE_ARCH" != "$(to_appimage_arch "$TARGET_ARCH_RESOLVED")" ]; then
    echo "Error: APPIMAGE_ARCH '$APPIMAGE_ARCH' does not match target '$TARGET_ARCH_RESOLVED'." >&2
    exit 1
fi
...
assert_safe_linux_work_dir "$APPIMAGE_WORK_DIR" "$PROJECT_ROOT" "$PUBLISH_DIR" "${DAEMON_DIR:-}"
