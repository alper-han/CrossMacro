...
mkdir -p "$DEB_DIR/usr/lib/systemd/system"
cp "$SCRIPTS_DIR/daemon/crossmacro.service" "$DEB_DIR/usr/lib/systemd/system/crossmacro.service"

# Copy Polkit Policy
echo "Copying Polkit Policy..."
mkdir -p "$DEB_DIR/usr/share/polkit-1/actions"
cp "$SCRIPTS_DIR/assets/io.github.alper_han.crossmacro.policy" "$DEB_DIR/usr/share/polkit-1/actions/io.github.alper_han.crossmacro.policy"

# Copy Polkit Rules
echo "Copying Polkit Rules..."
mkdir -p "$DEB_DIR/usr/share/polkit-1/rules.d"
cp "$SCRIPTS_DIR/assets/50-crossmacro.rules" "$DEB_DIR/usr/share/polkit-1/rules.d/50-crossmacro.rules"

# Copy udev rules
echo "Copying udev rules..."
cp "$SCRIPTS_DIR/assets/99-crossmacro.rules" "$DEB_DIR/usr/lib/udev/rules.d/99-crossmacro.rules"

# Copy modules-load config
echo "Copying modules-load config..."
mkdir -p "$DEB_DIR/usr/lib/modules-load.d"
cp "$SCRIPTS_DIR/assets/crossmacro-modules.conf" "$DEB_DIR/usr/lib/modules-load.d/crossmacro.conf"
