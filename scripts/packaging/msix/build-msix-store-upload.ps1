...
Builds Store-oriented MSIX artifacts from architecture-specific MSIX packages:
  ...
  - optionally creates CrossMacro-<PackageVersion>.appxsym from app PDBs kept outside shipped MSIX packages
  - creates CrossMacro-<PackageVersion>.msixupload containing the bundle and optional appxsym
...
Options:
  ...
  -SymbolsDir <dir>          Optional directory containing PDBs copied before MSIX cleanup.
  -BundlePath <path>         Output .msixbundle path. Defaults to <repo>/CrossMacro-<PackageVersion>.msixbundle.
  -AppxSymPath <path>        Output .appxsym path. Defaults to <repo>/CrossMacro-<PackageVersion>.appxsym.
...
function New-AppxSym {
    ...
    $pdbs = @(Get-ChildItem -LiteralPath $SourceDirectory -Recurse -File -Filter '*.pdb' -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -like 'CrossMacro*.pdb' })
    if ($pdbs.Count -eq 0) {
        Fail-MsixStoreBuild "no CrossMacro PDB files found for appxsym in $SourceDirectory"
    }
...
function Assert-ZipHasEntryMatching {
    param(
        [Parameter(Mandatory = $true)][string]$Path,
    ...
    try {
        $matchingEntries = @($zip.Entries | Where-Object {
            ...
            @($Pattern | Where-Object { $entryName -like $_ }).Count -gt 0
        })
        if ($matchingEntries.Count -eq 0) {
            Fail-MsixStoreBuild "$Path does not contain an entry matching any of: $($Pattern -join ', ')"
        }
    }
    finally {
        $zip.Dispose()
...
if (-not [string]::IsNullOrWhiteSpace($SymbolsDir)) {
    ...
    if (-not (Test-Path -LiteralPath $resolvedSymbolsDir -PathType Container)) {
        Fail-MsixStoreBuild "symbols directory not found: $resolvedSymbolsDir"
    }
}
...
$resolvedAppxSymPath = Resolve-OutputPath -Path $AppxSymPath -DefaultPath (Join-Path $projectRoot "CrossMacro-$PackageVersion.appxsym")
...
try {
    ...
    if ($resolvedSymbolsDir) {
        New-AppxSym -SourceDirectory $resolvedSymbolsDir -DestinationPath $resolvedAppxSymPath
        Assert-ZipHasEntryMatching -Path $resolvedAppxSymPath -Pattern @('CrossMacro*.pdb', '*/CrossMacro*.pdb')
    }
    ...
    if ($resolvedSymbolsDir) {
        Copy-Item -LiteralPath $resolvedAppxSymPath -Destination $uploadSourceDir -Force
    }
    ...
    if ($resolvedSymbolsDir) {
        $expectedUploadEntries += (Split-Path -Leaf $resolvedAppxSymPath)
    }
...
if ($resolvedSymbolsDir) {
    Write-Output "MSIX Store upload build: OK ($resolvedBundlePath, $resolvedAppxSymPath, $resolvedUploadPath)"
}
else {
