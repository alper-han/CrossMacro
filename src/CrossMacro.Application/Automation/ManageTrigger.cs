...
public sealed class ManageTrigger(ITriggerService service) : IManageTrigger
{
    private readonly ITriggerService _service = service ?? throw new ArgumentNullException(nameof(service));
    ...
    public async Task<TriggerTask> AddAsync(TriggerTask task, CancellationToken cancellationToken = default) { _ = await LoadAsync(cancellationToken).ConfigureAwait(false); cancellationToken.ThrowIfCancellationRequested(); _service.AddTask(task); cancellationToken.ThrowIfCancellationRequested(); await _service.SaveAsync().ConfigureAwait(false); return task; }
    public async Task<TriggerTask> UpdateAsync(TriggerTask task, CancellationToken cancellationToken = default) { _ = await LoadAsync(cancellationToken).ConfigureAwait(false); cancellationToken.ThrowIfCancellationRequested(); _service.UpdateTask(task); cancellationToken.ThrowIfCancellationRequested(); await _service.SaveAsync().ConfigureAwait(false); return task; }
