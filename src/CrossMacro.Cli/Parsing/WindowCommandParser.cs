...
internal static class WindowCommandParser
{
    public static CliParseResult Parse(string[] args)
    {
        ...
        return args[1].ToLowerInvariant() switch
        {
            ...
            "float" => ParseActiveFlag(args, "window.float", WindowCliAction.Floating),
    ...
    private static bool TryReadOperandInt(string[] args, ref int index, string name, out int? value, out string error)
    {
        ...
        index++;
        if (!int.TryParse(args[index], System.Globalization.CultureInfo.InvariantCulture, out var parsed))
        {
