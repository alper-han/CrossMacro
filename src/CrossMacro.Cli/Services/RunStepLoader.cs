    ...
    public static async Task<RunStepLoadOutcome> LoadAsync(RunCliExecutionRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.StepFilePath))
        {
            ...
            try
            {
                ...
                var fileStream = new FileStream(request.StepFilePath, FileMode.Open, FileAccess.Read, FileShare.Read, 65536, FileOptions.Asynchronous | FileOptions.SequentialScan);
