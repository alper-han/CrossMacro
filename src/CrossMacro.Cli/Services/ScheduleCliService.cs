...
public sealed class ScheduleCliService(IManageSchedule manageSchedule) : IScheduleCliService
{
    ...
    public async Task<CliCommandExecutionResult> ExecuteAsync(ScheduleCliOptions options, CancellationToken cancellationToken)
    {
        ...
        {
            ScheduleCliAction.Add => await AddAsync(options, cancellationToken).ConfigureAwait(false),
            ScheduleCliAction.Edit => await EditAsync(options, cancellationToken).ConfigureAwait(false),
            ScheduleCliAction.Remove => await RemoveAsync(options.TaskId ?? string.Empty, cancellationToken).ConfigureAwait(false),
            ScheduleCliAction.Enable => await SetEnabledAsync(options.TaskId ?? string.Empty, enabled: true, cancellationToken).ConfigureAwait(false),
            ScheduleCliAction.Disable => await SetEnabledAsync(options.TaskId ?? string.Empty, enabled: false, cancellationToken).ConfigureAwait(false),
            ScheduleCliAction.Next => await NextAsync(options.TaskId ?? string.Empty, cancellationToken).ConfigureAwait(false),
            _ => CliCommandExecutionResult.Fail(CliExitCode.InvalidArguments, "Unknown schedule action."),
        };
    }
    ...
    private async Task<CliCommandExecutionResult> AddAsync(ScheduleCliOptions options, CancellationToken cancellationToken)
    {
        ...
        cancellationToken.ThrowIfCancellationRequested();
        _ = await _manageSchedule.AddAsync(task, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<CliCommandExecutionResult> EditAsync(ScheduleCliOptions options, CancellationToken cancellationToken)
    {
        ...
        cancellationToken.ThrowIfCancellationRequested();
        _ = await _manageSchedule.UpdateAsync(task, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<CliCommandExecutionResult> RemoveAsync(string taskId, CancellationToken cancellationToken)
    {
        ...
        var task = parsed.Task!;
        cancellationToken.ThrowIfCancellationRequested();
        _ = await _manageSchedule.RemoveAsync(new TaskRequest(task.Id), cancellationToken).ConfigureAwait(false);
    ...
    private async Task<CliCommandExecutionResult> SetEnabledAsync(string taskId, bool enabled, CancellationToken cancellationToken)
    {
        ...
        cancellationToken.ThrowIfCancellationRequested();
        _ = await _manageSchedule.SetEnabledAsync(new TaskRequest(task.Id, enabled), cancellationToken).ConfigureAwait(false);
