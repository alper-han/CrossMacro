...
internal sealed class EditorScriptReader
{
    ...
    internal static bool TryParseWindowSearch(string step, out EditorAction action)
    {
        action = new EditorAction();
    ...
    internal static bool TryParseWindowWait(string step, out EditorAction action)
    {
        action = new EditorAction();
        var parts = step.Split(' ', 4, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        ...
        if (TryExtractLastToken(beforeVariable, out var beforeTimeout, out var maybeTimeout) && int.TryParse(maybeTimeout, NumberStyles.None, CultureInfo.InvariantCulture, out var parsedTimeout) && parsedTimeout > 0)
        {
    ...
    internal static bool TryParseWindowSelectorCommand(string step, WindowCommandMode mode, out EditorAction action)
    {
        action = new EditorAction();
        var parts = step.Split(' ', 4, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        if (parts.Length < 3)
    ...
    internal static bool TryExtractLastToken(string value, out string beforeLast, out string lastToken)
    {
