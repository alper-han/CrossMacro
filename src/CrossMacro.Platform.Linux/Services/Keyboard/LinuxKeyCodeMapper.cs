...
public class LinuxKeyCodeMapper : ILinuxKeyCodeMapper
{
    ...
    public string GetKeyName(int keyCode)
    {
        ...
        if (keyCode is >= 59 and <= 68)
        {
            return $"F{(keyCode - 58).ToString(CultureInfo.InvariantCulture)}";
        }
        ...
        if (keyCode is >= 183 and <= 194)
        {
            return $"F{(keyCode - 170).ToString(CultureInfo.InvariantCulture)}";
        }
    ...
    public int GetKeyCode(string keyName)
    {
        ...
        // Function keys
        if (keyName.Length > 0 && keyName[0] is 'F' or 'f' && int.TryParse(keyName[1..], CultureInfo.InvariantCulture, out var fNum))
        {
            if (fNum is >= 1 and <= 10)
            {
                return 59 + fNum - 1;
