...
internal static class ConsoleTestLock
{
    ...
    {
        if (!await Gate.WaitAsync(AcquireTimeout, CancellationToken.None))
        {
    ...
    private sealed class Releaser(IDisposable loggerScope) : IDisposable
    {
        private IDisposable? _loggerScope = loggerScope;
