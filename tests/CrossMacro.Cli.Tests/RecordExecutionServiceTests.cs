...
public sealed class RecordExecutionServiceTests
{
    ...
    {
        _ = _mousePositionProvider.IsSupported.Returns(returnThis: false);
        ConfigureImmediateStart();
        _ = _macroRecorder.IsRecording.Returns(returnThis: true, false);
        _ = _macroRecorder.StopRecording().Returns(CreateSequenceWithOneEvent());
...
recordKeyboard: true,
    ...
    {
        _ = _mousePositionProvider.IsSupported.Returns(returnThis: true);
        _ = _mousePositionProvider.GetAbsolutePositionAsync().Returns((100, 200));
        ConfigureImmediateStart();
        _ = _macroRecorder.IsRecording.Returns(returnThis: true, false);
        _ = _macroRecorder.StopRecording().Returns(CreateSequenceWithOneEvent());
...
recordKeyboard: true,
    ...
    public async Task ExecuteAsync_WhenRecordedSequenceFallsBackToRelative_DataReflectsEffectiveMode()
    {
        ...
        ConfigureImmediateStart();
        _ = _macroRecorder.IsRecording.Returns(returnThis: true, false);
        _ = _macroRecorder.StopRecording().Returns(new MacroSequence
        {
            Name = "recorded",
        ...
        Assert.Equal("relative", payload.ActualMode);
        Assert.True(payload.SkipInitialZero);
    }
    ...
    public async Task ExecuteAsync_WhenStartTaskDoesNotComplete_ReturnsCancelled()
    {
        ...
        _ = _macroRecorder.StartRecordingAsync(
                Arg.Any<bool>(),
                Arg.Any<bool>(),
                Arg.Any<IEnumerable<int>>(),
                Arg.Any<bool>(),
        ...
        _ = _macroRecorder.IsRecording.Returns(returnThis: false);
    ...
    public async Task ExecuteAsync_WhenCancelledBeforeStartAndStopFails_ReturnsRuntimeError()
    {
        ...
        _ = _macroRecorder.IsRecording.Returns(returnThis: true);
        _ = _macroRecorder.StopRecording().Returns(_ => throw new InvalidOperationException("stop failed"));
    ...
    public async Task ExecuteAsync_WhenCancelledBeforeStartAndRecorderStopsConcurrently_ReturnsCancelled()
    {
        ...
        _ = _macroRecorder.IsRecording.Returns(returnThis: true, false);
        _ = _macroRecorder.StopRecording().Returns(_ => throw new InvalidOperationException("Not currently recording"));
    ...
    public async Task ExecuteAsync_WhenDurationRequested_WaitsUntilStartupCompletesBeforeStopping()
    {
        ...
        _ = _macroRecorder.IsRecording.Returns(_ => isRecording);
        _ = _macroRecorder.StopRecording().Returns(unusedCallInfo =>
        {
            isRecording = false;
            _ = stopCalled.TrySetResult();
