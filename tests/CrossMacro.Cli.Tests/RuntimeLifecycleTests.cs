...
public sealed class RuntimeLifecycleTests
{
    ...
    public async Task CancelledStartRollsBackCompletedStepsWithoutStartingLaterSteps()
    {
        ...
        _ = await Assert.ThrowsAnyAsync<OperationCanceledException>(() => lifecycle.StartAsync(cts.Token));
    ...
    {
        var events = new List<string>();
        var lifecycle = new ApplicationRuntimeLifecycle([Step("first", events)]);
